...
4. 第一版 hidden 已提供 `stmem hidden` dry-run/apply。普通非核心事实只处理 coarse、importance 1–3、非 relation、无锚点且所有 core terms 在 archive 沉寂至少 90 天的摘要。动态副核心统一使用竞争性让位而非固定时间衰减：没有新高信息主线就无限期保留旧主线；新具体 core term 必须在至少 3 条 coarse、跨至少 2 天重复，并在 archive 跨至少 2 天出现，才算新主线站稳；旧摘要的 core terms 在新边界后至多 1 个活跃日且未被后续 coarse 沿用时才 hidden。relation 和锚点始终保护。写入 hidden 不调用模型并保持完整 content/coarse；历史无词摘要保留。暂不接 watcher、不做自动复活，先用新产生的 coarse 样本验证候选质量。
