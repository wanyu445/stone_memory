...
4. 第一版非核心 hidden 已提供 `stmem hidden` dry-run/apply：只处理 coarse、importance 1–3、非 relation、非当前副核心、无锚点且所有 core terms 在 archive 沉寂至少 90 天的摘要。写入 hidden 不调用模型并保持完整 content/coarse；历史无词摘要保留。暂不接 watcher、不做自动复活、不处理主副核心 hidden，先用新产生的 coarse 样本验证候选质量。
