...
新生成的 coarse 同时保存 1～3 个可回查的具体 `coreTerms`。普通非核心事实仅在无锚点、importance 1～3 且所有核心词至少沉寂指定天数时成为 hidden 候选，默认 90 天。当前动态副核心不按时间自然衰减：没有新主线就一直保留；只有新的具体核心词在至少 3 条、2 天 coarse 中站稳，且旧核心词在新主线形成后几乎消失，旧副核心 coarse 才竞争性让位。relation 始终排除。历史或用户手写 coarse 没有核心词时安全保留，不补跑模型；当前不实现自动复活，也未把 hidden 接入 watcher。
