...
新生成的 coarse 同时保存 1～3 个可回查的具体 `coreTerms`。第一版自动 hidden 只处理非 relation、非当前副核心、无锚点且 importance 1～3 的 coarse；所有核心词都在 archive 中至少沉寂指定天数才成为候选。默认 90 天，先 dry-run 后 apply。历史 coarse 或用户手写 coarse 没有核心词时安全保留，不为补词重新调用模型；当前不实现自动复活，也未把 hidden 接入 watcher。
